...
- **Domain detail page order**: CertificateCard → ShareButtons → AgeDisplay → RankingBar → Badge Value Props → BadgePreview → Claim CTA → DataSources → Timeline → Growth CTA. Badge value section shows 4 trust benefits (Build Trust, Boost Credibility, Strengthen SEO, Show Commitment). Claim CTA for unverified domains uses gold-bordered card with "Unlock the Gold Seal" messaging.
