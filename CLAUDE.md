...
- DNS verification DoH fallback: Queries Cloudflare DoH first (`cloudflare-dns.com`), falls back to Google DoH (`dns.google`) if no match. Handles multi-segment TXT records (`"seg1" "seg2"` → concatenated).
