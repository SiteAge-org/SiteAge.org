...
app.get("/og/:domain{[a-z0-9.-]+\\.[a-z]{2,}}", async (c) => {
  ...
  // 1. Check OG cache
  const cacheKey = `og:${domain}`;
  const cached = await c.env.BADGE_CACHE.get(cacheKey, "arrayBuffer");
  if (cached) {
  ...
  // 2. Get domain data (reuse badge's domain data fetch logic)
  ...
  if (domainCached) {
    ...
    try {
      ...
      console.error(`[OG] Failed to fetch domain data for ${domain}:`, err);
    }
  }
  ...
  await c.env.BADGE_CACHE.put(cacheKey, pngBuffer, { expirationTtl: BADGE_CACHE_TTL });
